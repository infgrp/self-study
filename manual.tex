...
\texttt{student\_rooms} & UNIQUE & \texttt{(study\_room\_id, seat\_number)} --- 같은 방 좌석 번호 중복 방지 (NULL 허용) \\
...
\texttt{migrate\_add\_constraints\_v2.py} & 데이터 무결성 제약 도입 이전 → 최신 &
  \texttt{users}·\texttt{attendance}·\texttt{student\_rooms} 테이블 재구축으로
  CHECK 제약(역할·성별·출결상태)과 UNIQUE 제약(자습실+좌석 번호) 추가 \\
\bottomrule
\end{tabularx}
\end{center}
...
\texttt{student\_rooms}
  & UNIQUE \texttt{(study\_room\_id, seat\_number)}
  & 같은 방에 같은 좌석 번호 중복 배정 차단 (\texttt{seat\_number}=NULL은 여러 행 허용) \\
\bottomrule
\end{tabularx}
\end{center}
