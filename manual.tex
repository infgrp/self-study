...
\begin{tipbox}
  로그 파일(\texttt{logs\textbackslash app.log})을 확인하면 서버 시작 메시지와
  오류 내용을 확인할 수 있습니다.
\end{tipbox}
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{학생 임시 비밀번호}: 비밀번호 해시가 없는 경우 \texttt{학번 + Study1}로 설정됩니다.\\
    (예: 학번 \texttt{10101} → 비밀번호 \texttt{10101Study1})
    \item \textbf{교사 비밀번호}: 백업 파일에 비밀번호 해시가 포함되어 있어 기존 비밀번호 그대로 복원됩니다.
    \item \textbf{자습실 복원 순서}: 자습실배정 시트를 복원하려면 자습실 목록이 먼저 복원되어 있어야 합니다. \textbf{자습실 목록}을 함께 선택하면 자동으로 먼저 처리됩니다. QR 토큰도 함께 복원되어 기존 QR코드가 그대로 사용됩니다.
    \item \textbf{순서}: 출결·신청·학습기록·자습실배정을 복원하려면 해당 학생 계정이 먼저 존재해야 합니다. 학생 계정 복원과 함께 선택하거나, 계정 복원 후 다시 실행하십시오.
    \item 같은 파일을 두 번 업로드해도 중복 생성되지 않으므로 안전합니다.
\end{itemize}
...
\textbf{조건} & \textbf{처리} \\
\midrule
해당 날짜 요일에 일정이 등록된 학생 & \badge{success}{출석} 처리 \\
자습 신청이 없는 경우 & 신청도 자동 생성 \\
이미 출결 기록이 있는 경우 & 건너뜀 (중복 생성 없음) \\
...
\begin{itemize}
  ...
  \item 복원된 학생의 초기 비밀번호는 \texttt{학번 + Study1} 형식입니다.
\end{itemize}
